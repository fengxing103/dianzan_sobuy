<template>
 ...
 -->
      <van-form @submit="onSubmit" style="margin-top: 40%;background-color: white;width: 96%;margin-left: 2%;border-radius: 20px;color: #000000;">
        <van-field
          type="text"
          autocomplete="off"
          style="height: 0;width: 0;padding: 0;position: absolute;color: #000000;"
        />
        <van-field
          type="password"
          autocomplete="off"
          ...
          <template #input>
            <van-dropdown-menu>
              <van-dropdown-item :title="`+${postData.dest}`" ref="DropdownItem">
                <template>
                  <ul>
                    <li v-for="item in areaList" :key="item.id" :class="{on:postData.dest==item.id}" @click="postData.dest=item.id,$refs.DropdownItem.toggle()">+{{item.id}}&#12288;{{item.name}}</li>
                  </ul>
                </template>
              </van-dropdown-item>
            </van-dropdown-menu>
            <input type="tel" v-model.trim="postData.username" :placeholder="$t('register.placeholder[0]')" style="border:0;flex:1;width:100px;padding-left: 50px;background: transparent;color: #000000;">
          </template>
        </van-field>
       ...
       <!-- <van-field
        ...
        >
          <template #button>
            <van-button size="mini" type="danger" round native-type="button" @click="getSmsCode" :loading="isSendCode" :loading-text="$t('register.text[1]')">{{$t('register.text[2]')}}</van-button>
          </template>
        </van-field> -->
